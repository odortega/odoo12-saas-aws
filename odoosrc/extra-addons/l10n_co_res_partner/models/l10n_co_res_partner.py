...
class CountryStateCity(models.Model):
    ...
    """
    _description = 'Modelo para manipular ciudades'
    _name = 'res.country.state.city'
    _order = 'code'

    code = fields.Char('Código Ciudad', size=5, help='Code DANE - 5 digits-',
                       required=True)
    name = fields.Char('Nombre Ciudad', size=64, required=True)
...
class PartnerInfoExtended(models.Model):
    ...
    companyName = fields.Char("Nombre de la Empresa")

    # Brand Name (e.j. Claro Móvil = Brand, COMCEL SA = legal name)
    companyBrandName = fields.Char("Marca")
    ...
    xidentification = fields.Char("Número de Documento", store=True,
    ...
    change_country = fields.Boolean(string="¿Cambiar País/Departamento?",
                                    default=True, store=False)

    # Name of point of sales / delivery contact
    pos_name = fields.Char("Nombre en Punto de Venta")

    # Birthday of the contact (only useful for non-company contacts)
    xbirthday = fields.Date("Cumpleaños")
    ...
    def _check_ident(self):
        ...
        for item in self:
            if item.doctype is not 1:
                msg = _('Error! El Número de Identificación debe tener'
                        'entre 2 y 12')
            ...
            if item.doctype is not 1:
                if item.xidentification is not False and \
                    ...
                    if re.match("^[0-9]+$", item.xidentification) is None:
                        msg = _('Error! Identificación solo puede '
                                'tener números')
                        raise exceptions.ValidationError(msg)
    ...
    def _checkDocType(self):
        ...
        if self.doctype is not 1:
            if self.doctype is False:
                msg = _('Error! Por favor seleccione un Tipo de Identificación')
                raise exceptions.ValidationError(msg)
            elif self.xidentification is False and self.doctype is not 43:
                msg = _('Error! Número de Identificación es obligatorio')
                raise exceptions.ValidationError(msg)
        ...
        if self.is_company is True:
            if self.personType is 1:
                if self.x_name1 is False or self.x_name1 == '':
                    msg = _('Error! Por favor ingrese el Nombre de la Persona')
                    raise exceptions.ValidationError(msg)
            elif self.personType is 2:
                if self.companyName is False:
                    msg = _('Error! Por favor ingrese el Nombre de la Empresa')
                    raise exceptions.ValidationError(msg)
    ...
    def _check_person_type(self):
        ...
        if self.personType is False:
            msg = _('Error! Por favor seleccione el Tipo de Persona')
            raise exceptions.ValidationError(msg)
